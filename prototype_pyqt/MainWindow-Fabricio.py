...
class MainWindow(QtWidgets.QMainWindow, QtWidgets.QApplication):
    ...
    def openFileNameDialog(self):
        ...
        fileName, _ = QFileDialog.getExistingDirectory(None, 'Select a folder:', 'C:\\', QtGui.QFileDialog.ShowDirsOnly)
        if fileName:
            print("Creating nifti file..." + fileName)
            ...
            for f in listdir(fileName):
                if isfile(join(fileName, f)):
                    ...
                    if ext == ".nii.gz" and "flair" in f:
                        print("Your file is..." + join(fileName, f))

        '''
        options = QFileDialog.Options()
                
        options |= QFileDialog.DontUseNativeDialog
        fileName, _ = QFileDialog.getOpenFileName(self,"Secuencia FLAIR", "../","DICOM files (*.dcm)", options=options)
        if fileName:
            print("Creating nifti file..." + fileName)
            self.dataset2 = pydicom.dcmread(fileName)
            print(self.dataset2)
            a = dicom2nifti.convert_directory(fileName, "../")
            print(a)'''

        #self.brain = setup_brain(self.renderer, "C:/Users/DELL/Desktop/TESIS/3d-nii-visualizer/sample_data/t1.nii.gz"), 
        #self.mask = setup_mask(self.renderer,"C:/Users/DELL/Desktop/TESIS/3d-nii-visualizer/sample_data/truth.nii.gz")
