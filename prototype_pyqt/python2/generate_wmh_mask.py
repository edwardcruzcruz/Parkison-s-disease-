...
def get_model_information():
    ...
    con = sql_connection()
    cursor = con.cursor()
    cursor.execute("SELECT * FROM model ORDER BY id DESC LIMIT 10;")
    theModels = cursor.fetchall()
    for model in theModels:
        print model
        path = model[4].encode('ascii', 'ignore')
        if path == '' or path == None:
            continue
        rows_standard = model[2]
        cols_standard = model[3]
        con.close()
        return path
...
def generateMask(FLAIR_image, model_path,size_image, resultPath):
    ...
    """ 
    if size_image == 35: imgs_test = Utrecht_preprocessing_flair(FLAIR_array)
    elif size_image == 63: imgs_test = GE3T_preprocessing_flair(FLAIR_array)
    else: imgs_test = FLAIR_array[..., np.newaxis]
    """
    flair_image = imgs_test[..., 0:1].copy()
    imgs_test = flair_image
    img_shape = (rows_standard, cols_standard, 1)
    model = get_unet(img_shape, first5=True)
    ...
    """ 
    if size_image == 35: original_pred = Utrecht_postprocessing(FLAIR_array, pred)
    elif size_image == 63: original_pred = GE3T_postprocessing(FLAIR_array, pred)
    else: original_pred = pred 
    """
...
if __name__ == "__main__":
    ...
    resultPath = sys.argv[2] #absolute path of the resulting image, ensure that all directories exist
    print(resultPath)
    model_path = get_model_information()
    if model_path == None: print 'some bad thing happen' 
    size_image = mask_X_size(FLAIR_image)
    generateMask(FLAIR_image, model_path, size_image, resultPath)