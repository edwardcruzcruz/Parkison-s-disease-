...
def train_leave_one_out(images, masks, patient=0, flair=True, t1=True, full=True, first5=True, aug=True, verbose=False):
    if full:
        if patient < 40:
            images = np.delete(images, list(range(patient*38, (patient+1)*38)), axis=0)
            masks = np.delete(masks, list(range(patient*38, (patient+1)*38)), axis=0)
        else:
            images = np.delete(images, list(range(1520+(patient-40)*63, 1520+(patient-39)*63)), axis=0)
            masks = np.delete(masks, list(range(1520+(patient-40)*63, 1520+(patient-39)*63)), axis=0)
    else:
        if patient < 20:
            images = images[:760, ...]
            masks = masks[:760, ...]
            ...
            images = np.delete(images, list(range((patient-20)*38, (patient-19)*38)), axis=0)
            masks = np.delete(masks, list(range((patient-20)*38, (patient-19)*38)), axis=0)
        else:
            images = images[1520:, ...]
            masks = masks[1520:, ...]
            images = np.delete(images, list(range((patient-40)*63, (patient-39)*63)), axis=0)
            masks = np.delete(masks, list(range((patient-40)*63, (patient-39)*63)), axis=0)
    if aug:
